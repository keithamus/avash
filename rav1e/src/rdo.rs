        ...
        for lru_y in 0..lru_h[pli] {
          ...
          for lru_x in 0..lru_w[pli] {
            ...
            if ts.restoration.has_restoration_unit(
              ...
              if let Some(taps) = fi.config.wiener {
                let c = taps[usize::from(pli > 0)];
                let filter = RestorationFilter::Wiener { coeffs: [c, c] };
                if best_lrf[lru_y * lru_w[pli] + lru_x][pli].notequal(filter) {
                  best_lrf[lru_y * lru_w[pli] + lru_x][pli] = filter;
                  lrf_change = true;
