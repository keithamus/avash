...
impl RestorationState {
  pub fn new<T: Pixel>(fi: &FrameInvariants<T>, input: &Frame<T>) -> Self {
    ...
    let (lrf_y_shift, lrf_uv_shift) = if fi.config.wiener.is_some() {
      (0, 0)
    } else if fi.sequence.enable_large_lru
    ...
    let lrf_type =
      if fi.config.wiener.is_some() { RESTORE_WIENER } else { RESTORE_SWITCHABLE };
    RestorationState {
      planes: [
